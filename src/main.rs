...
async fn main() -> Result<()> {
  ...
  tokio::spawn(async move {
    ...
    while let Some(message) = stream.next().await.transpose().unwrap() {
      if tx.send(format!("{message}")).await.is_err() {
  ...
  let app_result = App::new(rx).run(terminal);
  ratatui::restore();
  app_result
}
...
impl App {
  const fn new(receiver: tokio::sync::mpsc::Receiver<String>) -> Self {
    Self {
      input: String::new(),
  ...
  fn draw(&self, frame: &mut Frame) {
    let vertical = Layout::vertical([Constraint::Length(1), Constraint::Min(1), Constraint::Length(3)]);
